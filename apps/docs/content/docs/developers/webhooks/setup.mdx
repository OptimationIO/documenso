...
Webhooks are HTTP callbacks triggered by specific events in Aplyio. When an event occurs (such as a document being signed), Aplyio sends an HTTP POST request to your configured URL with details about the event.
...
Before configuring a webhook in Aplyio, you need an endpoint that can receive HTTP POST requests. Here's a minimal example:
...
<Callout type="warn">
  Always respond with a `200 OK` status within 30 seconds. Aplyio will retry failed deliveries.
</Callout>

## Configuring Webhooks in Aplyio via the Dashboard
...
Aplyio provides a built-in testing feature to verify your webhook endpoint works correctly.
...
<Step>
Aplyio will attempt to deliver the same payload again
</Step>
</Steps>

## Retry Policy

When a webhook delivery fails (non-2xx response or timeout), Aplyio automatically retries with exponential backoff:
...
<Accordions type="multiple">
  ...
  <Accordion title="Verify signatures">
    Use a webhook secret and verify the `X-Aplyio-Secret` header (see [Verification](/docs/developers/webhooks/verification)).
  </Accordion>
