...
Verifying webhook requests ensures that incoming payloads originate from Aplyio and have not been tampered with. Without verification, attackers could forge requests to your endpoint and trigger unintended actions.

## How Aplyio Signs Webhooks

When you configure a webhook with a secret, Aplyio includes that secret in every webhook request via the `X-Aplyio-Secret` header. Your server should compare this header value against your stored secret to authenticate the request.

```bash
POST /your-webhook-endpoint HTTP/1.1
Host: your-server.com
Content-Type: application/json
X-Aplyio-Secret: your_webhook_secret_here
...
| Header               | Description                                             |
| -------------------- | ------------------------------------------------------- |
| `X-Aplyio-Secret` | The secret key you configured when creating the webhook |
...
<Steps>
  <Step>
    Extract the `X-Aplyio-Secret` header from the incoming request
  </Step>
  <Step>
...
def handle_webhook():
    received_secret = request.headers.get('X-Aplyio-Secret', '')
...
| --------------- | --------------------------------------- | ------------------------------------------------------------------ |
| Secret mismatch | Webhook secret changed or misconfigured | Verify the secret in your environment matches the one in Aplyio |
| Empty header    | Webhook created without a secret        | Add a secret to the webhook configuration in Aplyio             |
...
3. Update the webhook configuration in Aplyio
