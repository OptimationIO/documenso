...
description: Create and send a document for signing using the Aplyio API, from uploading a PDF to adding recipients and distributing for signature.
---
...
- A Aplyio account (cloud or self-hosted)
...
| ----------- | -------------------------------------- |
| Production  | `https://app.aplyio.com/api/v2`     |
| Staging     | `https://stg-app.aplyio.com/api/v2` |
...
<Tab value="curl">
```bash
curl -X POST "https://app.aplyio.com/api/v2/envelope/envelope_abc123/distribute" \
  -H "Authorization: YOUR_API_TOKEN" \
...
const response = await fetch(
  `https://app.aplyio.com/api/v2/envelope/${envelopeId}/distribute`,
  {
    method: 'POST',
    headers: {
      Authorization: 'YOUR_API_TOKEN',
...
BASE_URL="https://app.aplyio.com/api/v2"
...
- [API Reference](https://openapi.aplyio.com/) - Full endpoint documentation with request/response schemas
