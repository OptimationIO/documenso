...
- A Aplyio account on any plan (Free, Individual, Team, or Enterprise)

<Callout type="info">
  Free accounts include API access with a limit of 5 documents per month. [Upgrade to a paid
  plan](https://aplyio.com/pricing) for higher limits.
</Callout>
...
```bash
curl https://app.aplyio.com/api/v2/documents \
...
```typescript
const response = await fetch('https://app.aplyio.com/api/v2/documents', {
  method: 'GET',
  headers: {
...
Aplyio provides official SDKs that handle authentication for you:

```typescript
import { Aplyio } from '@documenso/sdk-typescript';

const client = new Aplyio({
...
<Callout type="info">
  ...
  OpenAPI reference](https://app.aplyio.com/api/v1/openapi).
</Callout>
