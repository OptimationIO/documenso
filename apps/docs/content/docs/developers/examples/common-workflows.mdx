...
Register your endpoint in the Aplyio dashboard:
...
class AplyioApiError extends Error {
  code?: string;
  statusCode: number;
  details?: Record<string, unknown>;
...
try {
  ...
  if (error instanceof AplyioApiError) {
...
- [API Reference](https://openapi.aplyio.com/) - Full endpoint documentation
