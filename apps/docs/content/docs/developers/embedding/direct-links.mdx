...
When someone uses a direct link, Aplyio creates a new document from the template with that person as the signer.
...
curl -X POST "https://app.aplyio.com/api/v2/template/direct/create" \
...
// URL: https://app.aplyio.com/d/{token}
````
</Tab>
</Tabs>
...
```html
<iframe
  src="https://app.aplyio.com/embed/direct/abc123xyz"
  width="100%"
  height="800"
  frameborder="0"
...
function DocumentSigner({ token }: { token: string }) {
  return (
    <div className="h-[800px] w-full">
      <iframe
        src={`https://app.aplyio.com/embed/direct/${token}`}
...
function redirectToSigning(token: string) {
  window.location.href = `https://app.aplyio.com/d/${token}`;
}
```
...
```
https://app.aplyio.com/d/abc123xyz?externalId=order-12345
```
...
function getSigningUrl(token: string, orderId: string) {
  ...
  return `https://app.aplyio.com/d/${token}?${params.toString()}`;
}
...
```
https://app.aplyio.com/embed/direct/abc123xyz?externalId=order-12345
```
...
| Account   | Signer must be logged into a Aplyio account |
...
<Tab value="curl">
```bash
curl -X POST "https://app.aplyio.com/api/v2/template/direct/toggle" \
  -H "Authorization: api_xxxxxxxxxxxxxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    "templateId": 123,
...
```bash
curl -X POST "https://app.aplyio.com/api/v2/template/direct/delete" \
  -H "Authorization: api_xxxxxxxxxxxxxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    "templateId": 123
...
Enterprise plans support white-label embedding with the "Powered by Aplyio" badge removed. [Contact sales](mailto:support@aplyio.com) for details.
...
function getEmbedUrl(token: string, orderId: string) {
  ...
  return `https://app.aplyio.com/embed/direct/${token}?${params}`;
}
