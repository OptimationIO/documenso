...
S3 storage is recommended for production deployments. Aplyio supports AWS S3 and any S3-compatible storage service.
...
Using Docker Compose with Aplyio:
...
Aplyio works with any S3-compatible storage service. Configure the endpoint and enable path-style URLs if required.
...
- An S3 bucket configured for Aplyio
...
Aplyio does not provide automatic migration between storage backends. Each document's storage location is recorded in the database.
...
During migration, Aplyio can read from both backends. The `DocumentData.type` field indicates where each document is stored.
