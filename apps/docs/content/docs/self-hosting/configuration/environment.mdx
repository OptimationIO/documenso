...
description: Complete reference for all environment variables used to configure a self-hosted Aplyio instance.
---

## Required Variables

These variables must be set for Aplyio to function:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL of your Aplyio instance (e.g., `https://sign.example.com`)                   |
...
Aplyio requires PostgreSQL 14 or higher.
...
Aplyio supports multiple email transports for sending notifications.
...
Aplyio can store documents in the database or S3-compatible storage.
...
Aplyio requires a certificate to digitally sign documents.
...
Aplyio can use Google Vertex AI for recipient and field detection.
...
Aplyio supports multiple background job providers for processing emails, documents, webhooks, and scheduled tasks.
...
These variables require an active [Enterprise Edition](/docs/policies/enterprise-edition) license. Obtain a license key from [license.aplyio.com](https://license.aplyio.com) and set it below to unlock enterprise features such as SSO, embed authoring, and 21 CFR Part 11 compliance.
