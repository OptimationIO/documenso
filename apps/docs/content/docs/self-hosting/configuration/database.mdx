...
description: Configure PostgreSQL connection strings, pooling, SSL, migrations, and performance tuning for your Aplyio deployment.
---
...
Aplyio requires **PostgreSQL 14 or later**. No other databases are supported.
...
Aplyio uses two database connection variables:
...
Aplyio automatically detects common database environment variable formats used by hosting providers:
...
If your hosting provider sets these variables, Aplyio will use them automatically.
...
Aplyio uses Prisma, which maintains its own connection pool. Configure pool size with connection string parameters:
...
Database migrations update your schema when upgrading Aplyio.

### Automatic Migrations

When running Aplyio via Docker, migrations run automatically on container startup. No manual intervention is required.
...
Key parameters for Aplyio workloads:
...
Aplyio includes necessary indexes by default. Additional indexes may help for:
