...
Aplyio supports four email transport methods:
...
SMTP is the most common email transport. Aplyio supports two SMTP authentication methods.
...
Generate an API key in the Resend dashboard for Aplyio to use.
...
Aplyio uses React Email templates for all outgoing emails. The templates are located in `packages/email/templates/` and include:
...
- Custom logo replaces the Aplyio logo
...
Configure Aplyio to use the test server:
...
NEXT_PRIVATE_SMTP_FROM_NAME="Aplyio Test"
```
...
For MailChannels, configure DKIM directly in Aplyio using the environment variables described above.
