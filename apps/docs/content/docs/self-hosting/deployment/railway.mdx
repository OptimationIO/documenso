---
title: Railway
description: Deploy Aplyio on Railway with automatic builds from Git, managed PostgreSQL, and built-in networking. Use the one-click template or manual setup.
---
...
Click the URL in the Railway dashboard to access your Aplyio instance.
...
If you prefer more control over the setup, deploy Aplyio manually:
...
- Fork or select the [Aplyio repository](https://github.com/documenso/documenso)
...
Click on the Aplyio service and go to the "Variables" tab. Add the following variables:
...
NEXT_PRIVATE_SMTP_FROM_NAME=Aplyio
...
Aplyio runs database migrations automatically on startup.
...
Railway provides several options for scaling your Aplyio deployment:
...
<Steps>
<Step>
### Open your Aplyio service

In the Railway dashboard, select the Aplyio service.
...
In the Railway dashboard, select your Aplyio service and open "Settings".
...
<Callout type="warn">
  Aplyio stores sessions in the database, so horizontal scaling works without additional
...
Railway uses usage-based pricing. Estimated monthly costs for Aplyio:
...
Aplyio requires a signing certificate to sign documents. On Railway, you can:

### Option 1: Use Generated Certificate (Default)

Aplyio generates a self-signed certificate automatically if none is provided. This works for testing but shows generic certificate information in signed documents.
...
In your Aplyio service variables, set:
