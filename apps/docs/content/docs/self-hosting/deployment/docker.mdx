---
title: Docker
description: Run Aplyio as a standalone Docker container with an external database. Ideal for container platforms like AWS ECS, Google Cloud Run, or Azure Container Instances.
---
...
These variables must be set for Aplyio to start:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL where Aplyio is accessible (e.g., `https://sign.example.com`) | -                             |
...
Aplyio requires two database connection strings:
...
docker run -d \
  ...
  -e NEXT_PRIVATE_SMTP_FROM_NAME="Aplyio" \
...
Aplyio provides health check endpoints for monitoring:
...
To update Aplyio to a new version:
...
The Aplyio container is stateless. All persistent data is stored in the PostgreSQL database.
