...
## How Aplyio Supports Compliance

Aplyio provides features that support compliance with e-signature laws across jurisdictions:
...
Aplyio maintains an audit trail for each document, recording:
...
## What Aplyio Does NOT Provide

Aplyio supports compliance with Simple Electronic Signature (SES) requirements. The following are not currently provided:
