...
/**
 * GET /api/sign?template=TEMPLATE_ID&name=John+Doe&email=john@example.com
 *
 * Creates a document from a template with prefilled fields,
 * then redirects the signer directly to the signing page.
 *
 * Authentication: reads the Documenso API token from NEXT_PRIVATE_SIGN_API_TOKEN.
 * Requests that include `?token=` in the URL are rejected to prevent token leakage.
 *
 * Query params:
  ...
  .get('/', async (c) => {
    ...
    if (params.has('token')) {
      return c.json(
        {
          error:
            'Passing token in the URL is not allowed. The API token is now read from NEXT_PRIVATE_SIGN_API_TOKEN on the server.',
        },
        400,
      );
    }

    const apiToken = env('NEXT_PRIVATE_SIGN_API_TOKEN');

    if (!apiToken) {
      return c.json({ error: 'Server is missing NEXT_PRIVATE_SIGN_API_TOKEN' }, 500);
    }
      ...
      return c.json(
        {
          ...
          usage: 'GET /api/sign?template=TEMPLATE_ID&name=John+Doe',
        },
        400,
      );
    }
    ...
    try {
      ...
      if (!templateRes.ok) {
        ...
        return c.json(
          { error: 'Failed to fetch template', details: err },
          templateRes.status as 400,
        );
      }
      ...
      if (!sendRes.ok) {
        ...
        return c.json({ error: 'Failed to send document', details: err }, sendRes.status as 400);
      }
