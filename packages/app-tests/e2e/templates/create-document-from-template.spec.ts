...
test('[TEMPLATE]: should create a document from a template', async ({ page }) => {
  ...
  await page.getByLabel('Redirect URL').fill('https://aplyio.com');
  ...
  await page.getByPlaceholder('Email').nth(1).fill('recipient2@aplyio.com');
  ...
  expect(document.documentMeta?.redirectUrl).toEqual('https://aplyio.com');
...
test('[TEMPLATE]: should create a document from a template with custom document', async ({
  ...
  // Add a signer
  await page.getByPlaceholder('Email').fill('recipient@aplyio.com');
